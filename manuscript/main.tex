...
\vspace{1.5cm}
{\large Soumya Sekhar Banerjee$^{1}$ \quad and \quad Mehrdad Jalali$^{1,*}$\par}
\vspace{0.5cm}
{\normalsize $^{1}$SRH University of Applied Sciences Heidelberg, Heidelberg, Germany\par}
{\normalsize $^{*}$Corresponding author: \href{mailto:mehrdad.jalali@srh.de}{mehrdad.jalali@srh.de}\par}
\vspace{2cm}
\begin{abstract}
\noindent
Materials discovery in large quantum-chemical databases requires ranking candidates under partially competing objectives, incomplete descriptors, and diverse user intent, a challenge that conventional high-throughput screening addresses only partially. We present QMOF-Rec, a query-driven scientific recommender system that frames quantum-chemical MOF discovery as a multi-objective top-$K$ recommendation problem. Given a scientific query such as ``CO$_2$ adsorption MOFs'' or ``photocatalytic frameworks,'' QMOF-Rec retrieves a query-relevant candidate pool and applies a Lotus Effect Algorithm (LEA)-guided reranking layer to construct a final recommendation portfolio that balances relevance, property suitability, and intra-list diversity.
The system integrates semantic and metadata retrieval, property-aware scoring, formula-derived metadata-graph learning using GraphSAGE and graph attention networks (GAT), and retrieval-augmented generation (RAG) with large language model (LLM) explanation support. Saved offline evaluation artifacts on 20{,}372 QMOF vector metadata records across five query scenarios show that LEA preserves near-optimal relevance (Rel@5 = 0.6939) and ranking quality (NDCG@5 = 0.9959) while substantially improving intra-list diversity (0.1764); WeightedSum remains strongest on the aligned relevance metric. The revised repository handles descriptor availability with explicit masks: density is available for all 20{,}372 records, band gap for 10{,}810 records, and void fraction for none, so porosity is excluded from active numerical ranking, and new regression tests verify that unavailable descriptors are not interpreted as physical zero values. GraphSAGE/GAT results are reported as formula-derived metadata-graph baselines, not CIF-derived atomistic graph learning. A completed 30-query RAG/LLM evaluation across seven scientific scenarios shows perfect groundedness (5.0/5), zero metadata inconsistencies, a 3.3\% overall hallucination rate, and strong limitation-awareness and explanation quality under automated checks. No experimental, DFT, adsorption-uptake, or porosity validation is claimed.
...
QMOF-Rec moves beyond similarity retrieval toward multi-objective scientific recommendation. Its core novelty is the LEA-guided post-retrieval reranking layer, which treats the final recommendation list as a candidate portfolio. This allows QMOF-Rec to preserve high relevance while reducing redundancy and increasing intra-list diversity. To the best of our knowledge, QMOF-Rec is among the first frameworks to formulate quantum-chemical MOF discovery as a multi-objective, retrieval-augmented, graph-aware, and LEA-guided scientific recommendation task.
...
Table~\ref{tab:co2_example} reports the saved LEA top-5 recommendation output for the CO$_2$ adsorption scenario from the offline ranking artifact \texttt{lea\_evaluation/top\_rankings.csv}. The values should be interpreted under the metadata limitations above: density and band-gap values are available for these records, but void fraction is unavailable.

\begin{table}[H]
\centering
\caption{Example top-$K$ recommendations for the CO$_2$ adsorption scenario. Values are reproduced from the saved offline QMOF-Rec evaluation artifact. Void-fraction values are unavailable in the current metadata.}
...
\midrule
1 & qmof-f69d443 & Co$_2$C$_{40}$H$_{28}$N$_{12}$O$_8$ & 1.370 & 2.878 & 0.4808\\
2 & qmof-95efdd8 & Co$_2$C$_{50}$H$_{40}$N$_8$O$_8$    & 1.148 & 4.008 & 0.4801\\
3 & qmof-14eb177 & Co$_2$C$_{44}$H$_{36}$N$_8$O$_8$    & 1.411 & 3.497 & 0.4797\\
4 & qmof-f2d872e & Co$_2$C$_{36}$H$_{40}$N$_8$O$_{12}$ & 1.334 & 3.378 & 0.4781\\
5 & qmof-145a0bf & Co$_2$C$_{40}$H$_{28}$N$_4$O$_8$    & 1.525 & 2.997 & 0.4768\\
\bottomrule
\end{tabular}
\end{table}
...
The archived primary and repeated-seed results were generated using the historical evaluation protocol. The revised repository introduces mask-aware relevance, diversity, balance, and candidate remapping as a post-review implementation correction. Unless explicitly marked as a new mask-aware test, the numerical results reported below refer to the archived evaluation artifacts.

%% ════════════════════════════════════════════════════════════════════════════
\section{Recommendation Performance: Ranking Quality and Portfolio Diversity}
\label{sec:results}

Table~\ref{tab:ranking} reports the saved primary single-run aggregate top-$K$ recommendation results. WeightedSum achieves the highest mean relevance and NDCG because the relevance metric is aligned with weighted scoring. LEA is therefore not presented as universally outperforming WeightedSum in relevance. Its contribution is a different trade-off: WeightedSum is the strongest relevance-oriented ranker, but it produces a highly redundant list. LEA sacrifices only a small amount of relevance and NDCG while substantially improving intra-list diversity, making it more suitable for scientific candidate portfolios.

Figures~\ref{fig:comparison}--\ref{fig:convergence} summarize the archived evaluation from complementary perspectives. Figure~\ref{fig:comparison} compares aggregate quality metrics, Figure~\ref{fig:runtime} reports runtime on a logarithmic axis, and Figure~\ref{fig:convergence} shows LEA convergence behavior across query scenarios. Figure~\ref{fig:radar} illustrates the active objective representation used in the revised repository after excluding unavailable porosity information.

\begin{table}[H]
\centering
\caption{Aggregate ranking results from the saved primary single-run QMOF-Rec evaluation artifact. Higher is better for Rel@5, Diversity, NDCG@5, and the hypervolume proxy; lower is better for runtime. Bold denotes the best value and underline denotes the second-best value. Hypervolume is reported as a proxy rather than exact Pareto hypervolume.}
\label{tab:ranking}
\small
...
\midrule
WeightedSum    & \textbf{0.6961} & 0.0169 & \textbf{1.0000} & $3.46\times10^{-10}$ & 0.15\\
LEA            & \underline{0.6939} & \underline{0.1764} & \underline{0.9959} & \underline{$4.24\times10^{-10}$} & 126.28\\
TOPSIS         & 0.6936 & 0.0527 & 0.9952 & $\mathbf{4.79\times10^{-10}}$ & 0.20\\
ParetoCrowding & 0.6932 & \textbf{0.1831} & 0.9951 & $4.15\times10^{-10}$ & 34.63\\
Random         & 0.6830 & 0.0522 & 0.9746 & $3.01\times10^{-10}$ & \underline{0.14}\\
SemanticOnly   & 0.6818 & 0.0330 & 0.9702 & $3.04\times10^{-10}$ & \textbf{0.11}\\
\bottomrule
\end{tabular}
\end{table}
...
The main empirical conclusion is conservative. LEA preserves high relevance and NDCG while substantially improving diversity compared with WeightedSum. ParetoCrowding achieves the highest diversity but with slightly lower relevance than LEA. TOPSIS obtains the highest hypervolume proxy in the current setup. In recommender-system terms, LEA acts as a multi-objective reranker that improves list-level utility. Its value is not single-item optimality, but the construction of a top-$K$ portfolio that is relevant, less redundant, and more useful for downstream expert inspection.

\subsection{Example Recommendation Scenarios}

Table~\ref{tab:examples} reports the highest-ranked LEA recommendation for each evaluated query scenario from the saved offline ranking artifact. These examples illustrate the recommendation use case and should be read as offline recommendation outputs, not as experimentally validated discoveries.
...
\begin{table}[H]
\centering
\caption{Example LEA top-1 recommendations from the saved offline QMOF-Rec evaluation artifact. Void-fraction values are unavailable in the current metadata.}
\label{tab:examples}
\scriptsize
...
\midrule
CO$_2$ adsorption  & qmof-f69d443 & Co$_2$C$_{40}$H$_{28}$N$_{12}$O$_8$ & 2.878 & 1.370 & 0.4808 & Relevance, density, stability\\
Photocatalysis     & qmof-cef46c3 & Zn$_2$C$_{48}$H$_{32}$N$_8$O$_8$   & 2.913 & 0.790 & 0.8628 & Query match, band gap\\
Lightweight storage & qmof-b1cbe7a & Fe$_2$C$_{24}$H$_{16}$N$_8$        & 3.528 & 0.686 & 0.7467 & Low density, diversity\\
Balanced discovery  & qmof-b1cbe7a & Fe$_2$C$_{24}$H$_{16}$N$_8$        & 3.528 & 0.686 & 0.5981 & Broad objective profile\\
Insulating frameworks & qmof-017128d & Zn$_6$C$_{48}$H$_{60}$N$_{24}$   & 5.311 & 0.913 & 0.8944 & Wide band gap, stability\\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:ablation} reports the saved historical repeated-seed ablation and graph-aware recommendation results. These experiments compare deterministic rankers, MMR, LEA variants, graph-only ranking, and graph-augmented LEA using reproduced GraphSAGE/GAT embeddings from the formula-derived metadata graph. Table~\ref{tab:ranking} and Table~\ref{tab:ablation} therefore have different LEA diversity and runtime values because Table~\ref{tab:ranking} is the primary five-scenario single-run artifact, whereas Table~\ref{tab:ablation} is a ten-seed ablation artifact generated with the earlier configurable LEA script.

\begin{table}[H]
\centering
\caption{Saved historical ablation and graph-aware recommendation results. Values are means over repeated query/seed runs from the earlier configurable LEA protocol. Higher is better for Rel@K, NDCG@K, Diversity, and coverage; lower is better for runtime. The ``LEA no diversity'' row recovers the same saved aggregate values as WeightedSum in this configuration; this is a configuration-specific empirical identity, not a general equivalence claim. GraphSAGE/GAT variants use a formula-derived metadata graph, not a CIF-derived atomistic graph.}
\label{tab:ablation}
\scriptsize
...
\midrule
SemanticOnly      & 0.6818 & 0.9702 & 0.0330 & 1.000 & 0.09   & Vector/proxy retrieval only\\
WeightedSum       & 0.6961 & 1.0000 & 0.0169 & 1.000 & 0.13   & Deterministic weighted scoring\\
MMR               & 0.6885 & 0.9874 & 0.2105 & 1.000 & 2.66   & Diversity-aware reranking\\
LEA baseline      & 0.6939 & 0.9959 & 0.1662 & 1.000 & 129.17 & Historical baseline: $\beta=0.12$, $\gamma=0.08$\\
LEA no diversity  & 0.6961 & 1.0000 & 0.0169 & 1.000 & 124.19 & Diversity term disabled\\
LEA no self-clean & 0.6941 & 0.9963 & 0.1672 & 1.000 & 84.54  & Replacement disabled\\
GraphSAGE only    & 0.6840 & 0.9760 & 0.0514 & 1.000 & 0.04   & Graph similarity ranking\\
GAT only          & 0.6824 & 0.9749 & 0.0294 & 1.000 & 0.02   & Attention-embedding ranking\\
LEA + GraphSAGE   & 0.6930 & 0.9944 & 0.1795 & 1.000 & 120.31 & Graph-aware LEA signal\\
LEA + GAT         & 0.6913 & 0.9925 & 0.1916 & 1.000 & 93.48  & Graph-aware LEA signal\\
\bottomrule
\end{tabular}
\end{table}

The ablation results support the same conservative interpretation as Table~\ref{tab:ranking}. MMR is strong in diversity but weaker in relevance. LEA provides a more balanced relevance--diversity compromise for scientific candidate portfolios. In the saved aggregate artifacts, disabling diversity recovers the same aggregate relevance, NDCG, and diversity values as WeightedSum for the tested configuration; the result is reported as an empirical ablation outcome rather than as proof that the algorithms are generally equivalent. The saved metrics CSV does not include top-$K$ identifiers, so identity of the actual top-$K$ material IDs and ordering cannot be verified from that artifact. The graph-aware LEA variants increase diversity relative to WeightedSum and graph-only ranking, but they should not be overclaimed as physically validated until CIF-derived graph models and external validation are available. Full ablation outputs, including additional population sizes, iteration counts, candidate-pool sizes, and metric-level LEA-no-diversity checks, are reported in the supplementary material and reviewer revision artifact directory.

%% ════════════════════════════════════════════════════════════════════════════
\section{Statistical Analysis}
\label{sec:stats}

Repeated-run statistical summaries were computed on mean relevance over five query scenarios with ten random seeds per scenario. These 50 paired query/seed rows are useful for checking reranking robustness, but they should not be interpreted as 50 independent scientific queries because query-level variance may dominate seed-level variance. The primary statistical interpretation therefore treats query scenario as the blocking unit. At the query-block level, LEA baseline differs from the relevance-aligned WeightedSum configuration by a mean Rel@5 difference of $-0.0022$ (standard deviation across five query blocks $0.0030$; range $-0.0072$ to approximately $0.0000$). The historical Wilcoxon and Friedman tests over query/seed rows are retained only as descriptive ranking-metric checks in the supplementary artifact directory, not as evidence of physical material superiority.
...
A completed 30-query RAG/LLM answer-quality evaluation was conducted across seven scientific query scenarios: CO$_2$ adsorption, photocatalysis, lightweight gas storage, balanced discovery, wide band gap, general recommendation, and missing information trap. Retrieval used top-$k=5$ candidates per query. Responses were scored for retrieval relevance, groundedness, metadata consistency, limitation awareness, explanation quality, and binary hallucination flags. The detailed scoring rubric and flag definitions are reported in the supplementary material. The LLM was constrained not to claim experimental validation, CO$_2$ uptake, porosity performance, or CIF-derived graph evidence unless such evidence was present in retrieved context; these constraints were checked by the system prompt and post-hoc automated evaluators.

\subsection{Per-Scenario Results}

Table~\ref{tab:rag_results} reports evaluation results across all seven scenarios. The system achieves perfect scores on groundedness (5.0/5) and metadata consistency (5.0/5) across every scenario, confirming that the grounding rules and metadata-checking modules function correctly. The \texttt{wide\_band\_gap} scenario shows the lowest limitation-awareness score (3.0/5) because none of the four queries in that scenario triggered the missing-descriptor warning, as the band-gap descriptor is available for a substantial fraction of records.

\begin{table}[H]
\centering
\caption{Per-scenario RAG/LLM evaluation summary across 30 queries and 7 scenarios. All groundedness and metadata consistency scores are perfect (5.0/5).}
...
The saved repeated-seed artifacts add robustness experiments for LEA ablations, MMR, population size, iteration count, and candidate-pool size variants. Ten random seeds were used across five query scenarios, producing 50 query/seed rows per ablation variant; these rows are seed-repeated observations nested within five query scenarios, not 50 independent query tasks. NSGA-II, PSO, GA, and Bayesian optimization were not added to the completed-results table because no executable implementations for those baselines were available in the repository for this historical protocol. RAG explanation ablations were executed and their results are reported in Section~\ref{sec:rag_eval}.
...
The results illustrate why multi-objective recommendation is different from single-score screening. WeightedSum is fast and strongly aligned with the relevance metric, so it is expected to win on mean relevance. Scientific discovery, however, often benefits from a top-$K$ portfolio that spans multiple plausible regions of the materials space. LEA provides this list-level trade-off by preserving high relevance while improving intra-list diversity.

QMOF-Rec is an offline, query-driven hybrid scientific recommender with a strong content-based focus. Item representations are built from material metadata, query context, objective scores, graph-derived signals, and semantic retrieval rather than collaborative-filtering logs. LEA operates after candidate generation: the retrieved pool provides recall, deterministic scoring provides relevance and property suitability, and LEA reranking improves diversity and balance in the final recommendation list.
...
QMOF-Rec demonstrates that quantum-chemical MOF discovery can be framed as a query-driven scientific recommender-system problem. The evaluated LEA-guided reranker improves list-level diversity while maintaining high relevance and NDCG, making it suitable for constructing practical offline candidate portfolios for expert follow-up. WeightedSum remains strongest on the aligned relevance metric. A local formula-derived metadata-graph rerun also provides GraphSAGE/GAT property-prediction baselines and graph-aware recommendation variants; these are not CIF-derived atomistic graph-learning results. The platform architecture supports RAG-based scientific explanation.
